...
playersFileDirectory = '/Users/msharpe/src/personal/football-data-collection/footballData/footballData/spiders/players1_players_list_all.txt'
startIntFifa = 154994
...
def savePlayer(filename,count):
    ...
    with open(filePath,'r') as file_Player:
        ...
        try:
            cur.execute("INSERT INTO Player (player_api_id,player_fifa_api_id,player_name,birthday,height,weight) VALUES ( '%s', '%s', '%s', '%s', '%s', '%s' )" % ( player_api_id, player_fifa_api_id,player_name,birthday,height,weight) )
            print 'Inserted Player #' + str(count) +  ' - ' +filename
            conn.commit()

            cur.execute("SELECT id FROM Player WHERE player_api_id = '%s' " % (player_api_id))
        ...
        for node in allStatsNodes:
            ...
            cur.execute("INSERT OR IGNORE INTO Player_Stats (player_api_id,player_fifa_api_id,date_stat) VALUES ( '%s', '%s', '%s' )" % ( player_api_id, player_fifa_api_id, datePlayerStatUpdate) )
            cur.execute("SELECT id FROM Player_Stats WHERE player_api_id = '%s' AND date_stat = '%s'" % (player_api_id, datePlayerStatUpdate))
            ...
            for field in generalStatsField + detailedStatsField:
                ...
                try:
                    ...
                    return count
                try:
                    cur.execute("Update Player_Stats SET " + field + "='%s' WHERE id='%s'" % (stat,player_stats_id))
                except:
                    cur.execute("Update Player_Stats SET " + field + "='%s' WHERE id='%s'" % (None,player_stats_id))
                field_count += 1
